  ...
  for (const width of [1440, 390]) {
    ...
    await page.route('**/mock-api/**', async (route) => {
      ...
      if (path.endsWith('/delete')) {
        deletions++
        if (deletionFails)
          return route.fulfill({ status: 503, json: { error: { message: '删除服务暂时不可用' } } })
    ...
    await page.evaluate(() =>
      window.pushChatEvent({ event_type: 'chat_delta', text_delta: '第一段' }),
    )
    ...
    await page.evaluate(() =>
      window.pushChatEvent({ event_type: 'chat_delta', text_delta: '，第二段。' }),
    )
    await page.getByText('第一段，第二段。', { exact: true }).waitFor()
    await page.evaluate(() => {
      const call = {
        tool_call_id: 'inline-read',
        tool_call: { name: 'read_text_file', arguments: { path: 'README.md' } },
      }
      window.pushChatEvent({
      ...
      window.pushChatEvent({
        ...
        tool_call: call,
        tool_result: { tool_call_id: 'inline-read', tool_call_result: { content: '项目说明' } },
      })
      window.pushChatEvent({ event_type: 'chat_delta', text_delta: '读取完成，这是总结。' })
    })
    await page.getByText('读取完成，这是总结。', { exact: true }).waitFor()
    assert.equal(await page.locator('.chat-inline-tool').count(), 1)
    assert.deepEqual(
      await page
        .locator('.chat-transcript-list > li')
        .evaluateAll((items) =>
          items.map((item) =>
            item.querySelector('.chat-inline-tool')
              ? 'tool'
              : item.querySelector('.chat-message--assistant')
                ? 'assistant'
                : 'user',
          ),
        ),
    ...
    await page.locator('.chat-inline-tool > summary').click()
    await page
      .locator('.chat-inline-tool')
      .getByLabel('read_text_file 调用结果', { exact: true })
      .filter({ hasText: '项目说明' })
      .waitFor()
    await page.screenshot({ path: `${screenshots}/${width}-inline-tools.png` })
    run = {
      ...run,
      status: 'finished',
      ...
      response: {
        ...
        model_id: 'model-b',
        message: { role: 'assistant', content: [{ type: 'text', text: '读取完成，这是总结。' }] },
      },
    }
    ...
    await page.getByRole('button', { name: '发送', exact: true }).click()
    await page.waitForFunction(
      () => window.streamRequest.messages[0].content[0].text === 'another stream',
    )
    ...
    await page.evaluate(() =>
      window.pushChatEvent({ event_type: 'chat_delta', text_delta: '正在回答' }),
    )
    ...
    console.log(
      `${width}px: model selection, incremental stream, deduplication, deletion failure/cancel/success and persistence passed`,
    )
    await page.close()
  }
} finally {
