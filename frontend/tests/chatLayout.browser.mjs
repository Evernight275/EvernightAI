...
try {
  for (const [width, height] of [
    ...
    await page.route('**/mock-api/**', async (route) => {
      ...
      if (path === '/providers')
        return json([{ provider_id: 'test', name: 'Test', type: 'openai' }])
              ...
              content: [
                {
                  ...
                  text: [
                    '## 离散方程',
                    '',
                    String.raw`\[ -\frac{d}{dx}\left(kA\frac{dT}{dx}\right) = q A \]`,
                    '',
                    '```python',
                    'import numpy as np',
      ...
      if (path.endsWith('/retry/stream')) {
        run = {
          ...run,
          run_id: route.request().postDataJSON().retried_run_id,
          status: 'paused',
          stop_reason: null,
          pending_approval_requests: approvals,
        }
    ...
    await page.waitForFunction(
      (minimum) => document.querySelector('#chat-message').getBoundingClientRect().height > minimum,
      shortHeight,
    )
    await editor.fill('')
    await page.waitForFunction(
      (expected) =>
        document.querySelector('#chat-message').getBoundingClientRect().height === expected,
      shortHeight,
    )
    ...
    async function checkBounds() {
      const bounds = await page.evaluate(() => {
        ...
        return {
          header: rect('.chat-view-header'),
          body: rect('.chat-view-scroll'),
          footer: rect('.chat-view-footer'),
          editor: rect('#chat-message'),
    ...
    await page.waitForFunction(
      () => document.querySelector('.chat-header-status').textContent === '已取消',
    )
    ...
    assert.deepEqual(
      decisions[0].map((decision) => decision.status),
    ...
    if (width <= 760) {
      ...
      await page.waitForFunction(() => document.querySelector('.chat-sidebar-dialog').open)
      assert.equal(
        await page.locator('.chat-sidebar-dialog').evaluate((el) => el.matches(':modal')),
        false,
      )
    ...
    assert.ok(
      await page
        ...
        .innerText()
        .then((text) => text.includes('直接开始一段新对话。')),
    )
    await checkBounds()
    assert.deepEqual(errors, [])
    await page.route('**/mock-api/sessions', (route) =>
      route.fulfill({
        json: Array.from({ length: 40 }, (_, index) => ({
          ...session,
          session_id: `sidebar-${index}`,
          title: `会话 ${index + 1}`,
        })),
      }),
    )
